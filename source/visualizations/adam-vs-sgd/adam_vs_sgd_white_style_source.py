  ...
  <div id="mainWrap">
    ...
    <div class="panel" id="topPanel">
      <div id="plotTopRow">
        <div id="plotHeading">
          ...
          <p>
            ...
            The floor projection shows the (x, y) trajectory from above. The displayed &eta; values are
            surface-specific presets; the scale control multiplies them without changing the update equations.
          </p>
        </div>
      </div>
      ...
      <div id="controlWrap">
        ...
        <!-- Seek slider -->
        <div id="stepRow">
          <div class="smallLabel">Iteration</div>
        ...
        <div id="paramGrid">
          <div class="paramBox">
            <div class="paramHead">
              <div class="paramName">Learning-rate scale</div>
              <div class="paramVal" id="lrVal">1.00&times;</div>
            </div>
            <input class="range" id="lrSlider" type="range" min="50" max="150" step="5" value="100">
          </div>
          <div class="paramBox">
            <div class="paramHead">
          ...
          <div class="paramBox">
            <div class="paramHead">
              <div class="paramName">Animation detail</div>
              <div class="paramVal" id="stepsVal">120 frames</div>
            </div>
            <input class="range" id="stepsSlider" type="range" min="60" max="180" step="10" value="120">
          </div>
          <div class="paramBox">
            <div class="paramHead">
        ...
        <div id="toggleRow">
          ...
          <label class="check">
            Speed: <input class="range" id="speedSlider" type="range" min="80" max="600" step="20" value="140" style="width:90px; margin-left:6px;">
            <span id="speedVal" style="margin-left:6px; font-size:12px; color:#355fc1;">140 ms</span>
          </label>
        </div>
      </div>
    </div><!-- /panel -->
...
(function () {
  ...
  const SURFACES = {
    ...
    saddle: {
      label: 'Saddle',
      f:  (x,y) => x*x - y*y + 0.1*(Math.pow(x,4) + Math.pow(y,4)),
      g:  (x,y) => [2*x + 0.4*Math.pow(x,3), -2*y + 0.4*Math.pow(y,3)],
      start: [-2.5, 1.8],
      min:   [0, Math.sqrt(5)],
      xRange:[-3.5, 3.5],
      yRange:[-2.5, 2.5],
      N: 60,
      takeawayTitle: 'Saddle surface',
      takeaway: `At <strong>(0, 0)</strong>, curvature is upward along x and downward along y, so the origin is a
        true saddle rather than a minimum. From the selected start, the deterministic gradients lead toward
        the upper minimum at (0, &radic;5). Adam rescales each coordinate and SGD+Momentum accumulates
        velocity; no stochastic gradient noise is added in this simulation.`,
    },
  };

  // Surface-specific presets keep the raw optimizer equations stable and visible.
  // The learning rates are intentionally shown in the UI; no gradient or coordinate
  // clipping is used to force a trajectory to stay on the surface.
  const DEMO_PRESETS = {
    rosenbrock: {
      iterations: 5000,
      focusIterations: 30,
      tolerance: 0.05,
      learningRates: { adam: 0.05, sgd: 0.0015, sgdm: 0.0015 },
      logHeight: true,
    },
    ravine: {
      iterations: 300,
      focusIterations: 24,
      tolerance: 0.05,
      learningRates: { adam: 0.05, sgd: 0.18, sgdm: 0.08 },
      logHeight: false,
    },
    saddle: {
      iterations: 400,
      focusIterations: 24,
      tolerance: 0.05,
      learningRates: { adam: 0.03, sgd: 0.02, sgdm: 0.02 },
      logHeight: false,
    },
  };
  ...
  const state = {
    ...
    eps:      1e-8,
    maxSteps: 120,
    iterations: DEMO_PRESETS.rosenbrock.iterations,
    step:     0,
    playing:  false,
    animId:   null,
    lastTs:   null,
  ...
  function preset() {
    return DEMO_PRESETS[state.surface];
  }

  function learningRateFor(type) {
    return preset().learningRates[type] * state.lrScale;
  }

  function displayZ(rawZ) {
    return preset().logHeight ? Math.log1p(Math.max(0, rawZ)) : rawZ;
  }

  function iterationForFrame(frame) {
    if (frame <= 0) return 0;
    if (frame >= state.maxSteps) return state.iterations;

    const focusFrames = Math.min(30, Math.max(12, Math.floor(state.maxSteps / 4)));
    const focusIterations = Math.min(preset().focusIterations, state.iterations);
    if (frame <= focusFrames) {
      return Math.max(1, Math.round(frame * focusIterations / focusFrames));
    }

    const progress = (frame - focusFrames) / (state.maxSteps - focusFrames);
    return Math.round(focusIterations + (state.iterations - focusIterations) * progress * progress);
  }

  function pathIndexAt(path, iteration) {
    return Math.min(iteration, path.length - 1);
  }
  ...
  function buildSurf() {
    ...
    for (let j = 0; j < ys.length; j++) {
      ...
      for (let i = 0; i < xs.length; i++) {
        const rawZ = S.f(xs[i], ys[j]);
        const z = displayZ(rawZ);
        row.push(z);
        rawRow.push(rawZ);
        if (z < zMin) zMin = z;
        if (z > zMax) zMax = z;
    ...
    return { xs, ys, zs, rawZs, zMin, zMax };
  }

  // ── COMPUTE OPTIMIZER PATH ───────────────────────────────────────────────────
  function computePath(type) {
    const S   = SURFACES[state.surface];
    const lr  = learningRateFor(type);
    ...
    const pts = [{ t: 0, x, y, z: S.f(x,y) }];
    state.pathStatus[type] = null;

    for (let t = 1; t <= state.iterations; t++) {
      const [gx, gy] = S.g(x, y);
      if (!Number.isFinite(gx) || !Number.isFinite(gy)) {
        state.pathStatus[type] = { step: t, label: 'Diverged' };
        break;
      }
      ...
      if (x < S.xRange[0] || x > S.xRange[1] || y < S.yRange[0] || y > S.yRange[1]) {
        state.pathStatus[type] = { step: t, label: 'Out of view' };
        break;
      }
      pts.push({ t, x, y, z });
      if (Math.hypot(x - S.min[0], y - S.min[1]) < preset().tolerance) {
        state.pathStatus[type] = { step: t, label: 'Converged' };
        break;
      }
    }
    return pts;
  ...
  function buildAllPaths() {
    state.pathStatus = {};
    state.paths = {};
    state.paths.adam = computePath('adam');
    state.paths.sgd = computePath('sgd');
    state.paths.sgdm = computePath('sgdm');
  }

  // ── PLOTLY TRACES ─────────────────────────────────────────────────────────────
  function getTraces() {
    const step = state.step;
    const iteration = iterationForFrame(step);
    const previousIteration = iterationForFrame(Math.max(0, step - 1));
    const opt  = state.opt;
    ...
    traces.push({
      ...
      showscale: false,
      opacity: 0.97,
      hovertemplate: 'x=%{x:.2f}<br>y=%{y:.2f}<br>loss=%{customdata:.3f}<extra></extra>',
    ...
    for (const key of OPTS) {
      ...
      const index = pathIndexAt(path, iteration);
      const previousIndex = pathIndexAt(path, previousIteration);
      const slice = path.slice(0, index + 1);

      const px = slice.map(d => d.x);
      const py = slice.map(d => d.y);
      const pz = slice.map(d => displayZ(d.z));
      ...
      if (state.showProj && slice.length > 1) {
        ...
        traces.push({
          type: 'scatter3d', mode: 'lines',
          x: [cur.x, cur.x], y: [cur.y, cur.y], z: [ZF, displayZ(cur.z)],
      ...
      // Bright recent tail makes direction and movement clear while the full path stays visible.
      if (state.showSeg && index > 0) {
        const recentCount = Math.max(7, index - previousIndex + 1);
        const recent = path.slice(Math.max(0, index - recentCount), index + 1);
        traces.push({
          type: 'scatter3d', mode: 'lines',
          x: recent.map(d => d.x),
          y: recent.map(d => d.y),
          z: recent.map(d => displayZ(d.z)),
          line: { color, width: 10 },
          opacity: 0.98,
          hoverinfo: 'skip',
          showlegend: false,
      ...
      const cur = slice[slice.length - 1];
      const status = state.pathStatus[key];
      const isConverged = status && status.label === 'Converged' && iteration >= status.step;
      const textPositions = { adam: 'top left', sgd: 'top right', sgdm: 'bottom center' };
      const symbols = { adam: 'circle', sgd: 'square', sgdm: 'diamond' };
      traces.push({
        type: 'scatter3d', mode: 'markers',
        x: [cur.x], y: [cur.y], z: [displayZ(cur.z)],
        marker: { size: 17, color, opacity: 0.20 },
        hoverinfo: 'skip',
        showlegend: false,
      });
      traces.push({
        ...
        x: [cur.x], y: [cur.y], z: [displayZ(cur.z)],
        text: [`${name}${isConverged ? ' \u2713' : ''}`],
        textposition: textPositions[key],
        textfont: { size: 13, color, family: 'Inter, sans-serif' },
        marker: { size: 11, color, symbol: symbols[key], line: { color: '#fff', width: 3 } },
        name,
    ...
    // All optimizers share the same initial parameters, so one start marker avoids overlap.
    const S = SURFACES[state.surface];
    const [sx, sy] = S.start;
    traces.push({
      type: 'scatter3d', mode: 'markers+text',
      x: [sx], y: [sy], z: [displayZ(S.f(sx, sy))],
      text: ['start'],
      textposition: 'bottom left',
      textfont: { size: 11, color: '#b56b00', family: 'Inter, sans-serif' },
      marker: { size: 7, color: '#f59e0b', symbol: 'circle', line:{ color:'#fff', width:2 } },
      hovertemplate: 'Shared start<br>x=%{x:.3f}, y=%{y:.3f}<extra></extra>',
      showlegend: false,
    });
  ...
  function getLayout() {
    ...
    return {
      ...
      transition: { duration: Math.min(120, Math.round(state.speedMs * 0.65)), easing: 'cubic-in-out' },
      scene: {
        xaxis: {
        ...
        zaxis: {
          title: { text: preset().logHeight ? 'display: log(1 + loss)' : 'loss f(x,y)', font:{ size:14, color:'#263b6b' } },
  ...
  function fmtLoss(v) {
    ...
    return magnitude > 0 && magnitude < 0.01
      ? v.toExponential(2)
      : magnitude < 10 ? v.toFixed(4) : v.toFixed(1);
  }
  ...
  function pathStatusAt(type, iteration) {
    const status = state.pathStatus[type];
    return status && iteration >= status.step ? status.label : '';
  }

  function updateRulesMeta() {
    const rates = ['adam', 'sgd', 'sgdm']
      .map(type => `${type === 'sgdm' ? 'SGD+M' : type.toUpperCase()} \u03b7=${fmtEta(learningRateFor(type))}`)
      .join('  \u00b7  ');
    document.getElementById('rulesMeta').textContent = rates;
  }
  ...
  function updateUI() {
    ...
    const ap = state.paths.adam[pathIndexAt(state.paths.adam, iteration)];
    const sp = state.paths.sgd[pathIndexAt(state.paths.sgd, iteration)];
    const mp = state.paths.sgdm[pathIndexAt(state.paths.sgdm, iteration)];
    const adamStatus = pathStatusAt('adam', iteration);
    const sgdStatus = pathStatusAt('sgd', iteration);
    const sgdmStatus = pathStatusAt('sgdm', iteration);

    const lossText = (status, point) => status === 'Converged'
      ? `\u2713 ${fmtLoss(point.z)}`
      : status || fmtLoss(point.z);
    const distText = (status, point) => status === 'Converged'
      ? `\u2713 ${fmtDist(point.x,point.y,mx,my)}`
      : status || fmtDist(point.x,point.y,mx,my);

    document.getElementById('stepMeta').textContent    = `iteration ${iteration.toLocaleString()} of ${state.iterations.toLocaleString()}`;
    document.getElementById('stepValue').textContent   = `${iteration.toLocaleString()} / ${state.iterations.toLocaleString()}`;
    document.getElementById('seekSlider').value        = step;

    document.getElementById('mAdam').textContent     = lossText(adamStatus, ap);
    document.getElementById('mSgd').textContent      = lossText(sgdStatus, sp);
    document.getElementById('mSgdm').textContent     = lossText(sgdmStatus, mp);
    document.getElementById('mDistAdam').textContent = distText(adamStatus, ap);
    document.getElementById('mDistSgd').textContent  = distText(sgdStatus, sp);
    document.getElementById('mDistSgdm').textContent = distText(sgdmStatus, mp);
    updateRulesMeta();
  }
  ...
  window.setSurface = function(s) {
    ...
    buildAllPaths();
    document.getElementById('lrSlider').value = '100';
    document.getElementById('lrVal').innerHTML = '1.00&times;';
    document.getElementById('surfVal').textContent = SURFACES[s].label.slice(0,6);
    [['rosenbrock','surfRosen'], ['ravine','surfRavine'], ['saddle','surfSaddle']].forEach(([k,id]) => {
  ...
  function init() {
    ...
    document.getElementById('stepsVal').textContent = `${state.maxSteps} frames`;
    ...
    document.getElementById('lrSlider').addEventListener('input', e => {
      state.lrScale = Number(e.target.value)/100;
      document.getElementById('lrVal').textContent = `${state.lrScale.toFixed(2)}\u00d7`;
      rebuildPaths();
    });

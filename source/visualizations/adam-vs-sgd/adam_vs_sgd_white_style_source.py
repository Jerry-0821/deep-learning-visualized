...
(function () {
  ...
  function pathStatusAt(type, step) {
    const status = state.pathStatus[type];
    return status && step >= status.step ? status.label : '';
  }
  ...
  function updateUI() {
    ...
    const adamStatus = pathStatusAt('adam', step);
    const sgdStatus = pathStatusAt('sgd', step);
    const sgdmStatus = pathStatusAt('sgdm', step);
    ...
    document.getElementById('mAdam').textContent    = adamStatus || fmtLoss(ap.z);
    document.getElementById('mSgd').textContent     = sgdStatus || fmtLoss(sp.z);
    document.getElementById('mSgdm').textContent    = sgdmStatus || fmtLoss(mp.z);
    document.getElementById('mDistAdam').textContent = adamStatus || fmtDist(ap.x,ap.y,mx,my);
    document.getElementById('mDistSgd').textContent  = sgdStatus || fmtDist(sp.x,sp.y,mx,my);
    document.getElementById('mDistSgdm').textContent = sgdmStatus || fmtDist(mp.x,mp.y,mx,my);
  }
